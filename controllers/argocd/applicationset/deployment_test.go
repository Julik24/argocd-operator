...
func TestApplicationSetReconciler_reconcileDeployment(t *testing.T) {
	...
	//reconciler := makeTestApplicationSetReconciler(t, false, ns)
	//reconciler.varSetter()
	//reconciler.getCmd()
	//err := reconciler.getdesired()
	//assert.NoError(t, err)
	//existingDeployment := reconciler.getdesired()
	//t.Errorf("Expected error but did not get one %v", existingDeployment)
	tests := []struct {
		name        string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := makeTestApplicationSetReconciler(
				t, false,
			)
			rec.varSetter()
			err := rec.reconcileClusterRole()
